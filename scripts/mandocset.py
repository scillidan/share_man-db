...
def getPlist(identifier: str, bundle_name: str) -> str:
    lines = [
...
class DocsetMaker:
    ...
    def __init__(
        self, outname, executable: str, identifier: str = "", bundle_name: str = ""
    ):
        self.outname = outname
        self.identifier = identifier or outname.split("_")[0]
        self.bundle_name = bundle_name or outname.replace("_", " ")
        self.dups = set()
        self.db = None
    ...
    def __enter__(self):
        ...
        self.db = sqlite3.connect(
            self.outname + ".docset/Contents/Resources/docSet.dsidx"
        )
        self.db.execute(
        ...
        with open(self.outname + ".docset/Contents/Info.plist", "w") as plist:
            plist.write(getPlist(self.identifier, self.bundle_name))
        return self
...
def main():
    ...
    argp.add_argument(
        "-p",
        "--paths",
        help="paths with unpacked archive, order matters",
        nargs="+",
        required=True,
    )
    ...
    argp.add_argument(
        "--identifier",
        help="CFBundleIdentifier (defaults to first segment of outname split by _)",
    )
    argp.add_argument(
        "--bundle-name",
        help="CFBundleName (defaults to outname with _ replaced by spaces)",
    )
    ...
    argp.add_argument(
        ...
        default="man2html -r",
        help=(
            "Executable with arguments which reads from stdin and writes to stdout."
    ...
    with DocsetMaker(
        args.out, args.e, args.identifier or "", args.bundle_name or ""
    ) as dsm:
